...
/*func SendToOther(currentConn *net.TCPConn, data []byte) {

	map_lock.lock.Lock()

	for conn := range map_lock.cli_conn_map {
		if conn != currentConn {
			conn.Write(data)
		}
	}
	...
	seq := uint16(uint16(data[0])<<8 + uint16(data[1]))
	msg_size := uint16(uint16(data[2]<<8) + uint16(data[3]))
	fmt.Printf("server send to all cli_%d msg ::%s \n", seq, string(data[4:msg_size+4]))
}

func RChannel2WChannel(r, w chan byte) {
...
func Clear(data []byte, size int) {
	for i := 0; i < size; i++ {
		data[i] = 0
	}
}

func CopyData(pFrom []byte, pTo []byte) {
